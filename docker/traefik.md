...
6. _Because eero (or something else) hammers my pihole dns server,_ add an entry
...
## Tightening up traefik on HTTPS

In CLI `docker network create proxy-net` first to create the network manually. something I will always forget.
Then see the compose files for changes that I should have documented here.

see <./docker-socket-proxy.md> for next steps to secure things better.
